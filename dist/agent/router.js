...
const DEFLECTION = "Sorry, I can only help with transfer-tracking questions today. I’m connecting you to a human support agent.";
...
function hasOutOfScopeSignals(q) {
    const outSignals = [
        "cancel",
        "refund",
        "fee",
...
function detectIntent(question) {
    const q = normalizeQuestion(question);
    if (!q)
        return { intent: null, confidence: 0, reason: "Empty question" };
...
async function routeQuestion(opts) {
    ...
    return {
        kind: "IN_SCOPE",
        answer: (0, faq_1.buildAnswer)(intent.intent),
        closingMessage: CLOSING,
        debug: { normalizedQuestion, scope, intent }
    };
}
